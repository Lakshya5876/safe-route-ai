...
// Must match RiskModelService's SUPPORTED_CITIES in the backend — SafeRoute
// only has real risk-grid data for these six cities (see pipeline/README.md).
...
const COVERAGE_LABEL = {
  high: "Good data coverage",
  moderate: "Moderate data coverage",
  sparse: "Sparse data — treat as approximate",
  insufficient: "Insufficient data for this area",
};
...
function RouteInfoPanel({ routes = [], selectedRouteId, onSelectRoute, route }) {
  ...
  const confidencePct = Math.round((route.confidence ?? 0) * 100);
  const coverageTier = route.coverageTier ?? "insufficient";
  ...
  return (
    ...
    >
      ...
      <div style={routeInfoRow}>
        <span style={routeInfoLabel}>Confidence</span>
        <span style={routeInfoValue}>{confidencePct}%</span>
      </div>
      <div style={coverageBadge(coverageTier)}>{COVERAGE_LABEL[coverageTier] ?? coverageTier}</div>
      {route.paretoOptimal && !route.primary && (
        <div style={paretoBadge}>Pareto-optimal alternative — no other option is both safer and faster</div>
      )}
      {route.generationMethod === "ORS_RISK_AVOIDANCE" && (
        <div style={paretoBadge}>Generated by actively routing around a high-risk area</div>
      )}
...
function coverageBadge(tier) {
  ...
  return {
    ...
    border: `1px solid ${colors[tier] ?? "#94a3b8"}`,
  };
}
...
export default function Home() {
  ...
  async function handleRouteSearch() {
    try {
      ...
      if (!city) {
        alert("Please select a city. SafeRoute only has real risk data for the six supported cities.");
        return;
      }

      setLoading(true);

      // 🔹 1. Convert text → coordinates using Mapbox, biased toward the selected city
      const originCoords = await geocodeLocation(origin, city);
      const destinationCoords = await geocodeLocation(destination, city);
